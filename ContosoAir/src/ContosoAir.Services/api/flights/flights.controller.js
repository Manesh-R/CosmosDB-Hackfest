...
let get = (req, res) => {
    ...
    if (config.API_MODE == "MONGO") {
        console.log('flights:get:LocalMongoDB');

        MongoClient.connect(config.MONGO_DB_CONNECTION_STRING, function (err, db) {
            if (err) {
                res.send(err);
